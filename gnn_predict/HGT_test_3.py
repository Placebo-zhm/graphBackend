...
# 修改保存函数
def save_hetero_data(data, nodes, edges, path="hetero_data.pt"):
    """保存异构数据及原始节点信息"""
    save_dict = {
        'hetero_data': data,
        'nodes': nodes,
...
# 修改加载函数
def load_hetero_data(path="hetero_data.pt"):
    """加载预处理数据及节点信息"""
    try:
        save_dict = torch.load(path)
        print("成功加载预处理数据")
        return save_dict['hetero_data'], save_dict['nodes'], save_dict['edges']
    except Exception as e:
        print(f"加载失败: {str(e)}")
...
def visualize_embeddings(z_dict, labels, n_clusters=5):
    ...
    # 自动确定聚类数量
    if n_clusters == 'auto':
        # 动态调整max_k，基于数据量设置更高上限
        max_k = min(100, int(len(embeddings) * 0.1))  # 取数据量的10%或100的较小值
        max_k = max(50, max_k)  # 确保最低探索50个聚类
        ...
        elbow_k = np.argmin(second_diff) + 2  # +2补偿两次差分偏移

        # 优先考虑肘部法则结果
        n_clusters = elbow_k

        # 解除最终约束上限
        n_clusters = max(2, n_clusters)  # 仅保留下限约束
        print(f"自动确定的聚类数量: {n_clusters}")

    # t-SNE参数
    tsne = TSNE(n_components=2, perplexity=16, n_iter=1000, random_state=42)
...
if __name__ == "__main__":
    ...
    data_path = "hetero_data.pt"
    hetero_data, pre_nodes, pre_edges = load_hetero_data(data_path)

    if hetero_data is None:
        print("重新构建异构数据...")
        nodes, edges = load_data('../data/jsondata/relations.json')
        hetero_data = build_hetero_data(nodes, edges)
        save_hetero_data(hetero_data, nodes, edges, data_path)  # 保存完整数据
    else:
        # 使用预处理保存的节点信息
        nodes = pre_nodes
        edges = pre_edges
